...
当前源码主线的截图流程是：完成系统截图后先进入独立编辑窗口，而不是直接复制。用户可在编辑器中二次裁剪、绘制矩形/椭圆/直线/箭头、插入可旋转文字、打马赛克，或进入 OCR 模式在图片原位拖选文字；确认后才把最终 PNG 写入剪贴板并更新最近三张历史。历史卡片的“打开”已改为“编辑”，确认后原位替换对应历史项，不新增条目。
