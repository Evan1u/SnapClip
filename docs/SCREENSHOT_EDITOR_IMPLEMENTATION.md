...
本期不实现：固定比例/比例预设/自动主体裁剪、自由画笔、高亮、编号、贴纸、阴影、任意数值或连续滑杆式线宽调节、图形与马赛克旋转、多选、图层面板、持久化可编辑工程、多个并行编辑窗口、云端 OCR、滚动截图、录屏。

新截图在屏幕原位置的全屏 overlay 编辑属于后续独立工作，见 `docs/INPLACE_EDITOR_IMPLEMENTATION.md`；历史卡片编辑仍沿用本文档所述独立编辑窗口语义。
